...
import { chartEntries, families, snippetOf, type ChartEntry, type Family } from '../lib/charts';
...
<template>
    <main class="charts-page">
        <div class="doc-head">
            ...
            <h1>{{ chartEntries.length }} charts, drawn without a dependency</h1>
            <p>
        ...
        <section v-for="group in groups" :key="group.name" class="charts-group" :aria-labelledby="`charts-${group.name.replace(/\s/g, '-')}`">
            ...
            <div class="charts-grid">
                <article v-for="entry in group.items" :key="entry.id" class="charts-card" :aria-labelledby="`chart-${entry.id}`">
                    ...
                    <p>{{ entry.note }}</p>
                    <Button
                        :label="open === entry.id ? 'Hide the code' : 'Show the code'"
        ...
        <section class="charts-more" aria-labelledby="charts-more">
            <h2 id="charts-more">Beyond the shapes</h2>
            <ul>
                <li><Icon icon="search" /> <span><b>Zoom and brush.</b> Drag on the plot to zoom, or pair a chart with a brush chart that selects the window.</span></li>
                <li><Icon icon="grip" /> <span><b>Synced groups.</b> Charts sharing a <code>group</code> move their tooltip and their window together.</span></li>
