...
const categoryOptions = computed(() => [
    { label: t('All categories ({count})', { count: iconList.length }), value: 'all' },
    ...categories.map((c) => ({ label: `${t(c.label)} (${c.icons.length})`, value: c.id }))
]);

// Words are matched independently against the name (split at its capitals),
// the category (in English and in the page's language) and the tags, so
// "cart plus" finds `cartPlus`.
const haystack = computed(
    () =>
        new Map(
            iconList.map((def) => {
                const label = labelOf.get(def.category ?? '') ?? '';
                return [def, [def.name.toLowerCase(), def.name.replace(/([A-Z])/g, ' $1').toLowerCase(), def.category ?? '', label, label && t(label), ...(def.tags ?? [])].join(' ').toLowerCase()];
            })
        )
);
...
const groups = computed(() => {
    ...
    return categories
        ...
        .map((c) => ({ ...c, icons: c.icons.filter((def) => words.every((word) => haystack.value.get(def)!.includes(word))) }))
...
async function copy(what: 'name' | 'snippet' | 'svg') {
    ...
    announcement.value = '';
    const said = {
        name: ok ? t('Copied the name of {name}.', { name: def.name }) : t('Could not copy the name; select it from the panel instead.'),
        snippet: ok ? t('Copied the Icon snippet of {name}.', { name: def.name }) : t('Could not copy the Icon snippet; select it from the panel instead.'),
        svg: ok ? t('Copied the SVG of {name}.', { name: def.name }) : t('Could not copy the SVG; select it from the panel instead.')
    }[what];
    requestAnimationFrame(() => (announcement.value = said));
}
...
/** Where the selected icon is, by name, when it is not one of the base set. */
const byName = computed(() =>
    selected.value
        ? t('By name, <code>icon="{name}"</code>, after <code>registerIcons([{name}])</code>, or the plugin\'s <code>icons</code> option.', { name: selected.value.name })
        : ''
);
...
<template>
    <main class="icons-page" :style="{ '--icons-size': `${size}px` }">
        <div class="doc-head">
            <span class="eyebrow">{{ t('Icons') }}</span>
            <h1>{{ t('{count} icons, drawn for Vitral', { count: iconList.length }) }}</h1>
            <T k="iconsPage.lead">
        ...
        <div class="icons-toolbar" role="search">
            <div class="icons-field icons-search">
                <label :for="ids.search">{{ t('Search') }}</label>
                <IconField>
                    <InputIcon icon="search" />
                    <InputText :id="ids.search" v-model="query" type="search" :placeholder="t('Name, category or keyword')" fluid />
                </IconField>
            </div>
            <div class="icons-field">
                <label :for="ids.category">{{ t('Category') }}</label>
                <Select :id="ids.category" v-model="category" :options="categoryOptions" option-label="label" option-value="value" fluid />
            </div>
            <div class="icons-field">
                <span :id="ids.size" class="icons-label">{{ t('Size') }} <output>{{ size }}px</output></span>
                <div class="icons-control">
                    <Slider v-model="size" :min="16" :max="48" :step="4" :aria-labelledby="ids.size" :format-value="(v: number) => t('{count} pixels', { count: v })" />
                </div>
            </div>
            <div class="icons-field">
                <span :id="ids.stroke" class="icons-label">{{ t('Stroke') }} <output>{{ stroke }}</output></span>
                <div class="icons-control">
        ...
        <p class="icons-count" role="status" aria-live="polite">
            {{ count === iconList.length ? t('{count} icons', { count }) : t('{count} of {total} icons', { count, total: iconList.length }) }}
        </p>
        ...
        <div class="icons-layout">
            <div class="icons-results">
                <section v-for="group in groups" :key="group.id" class="icons-group" :aria-labelledby="`icons-${group.id}`">
                    <h2 :id="`icons-${group.id}`">{{ t(group.label) }} <small>{{ group.icons.length }}</small></h2>
                ...
                <div v-if="!count" class="icons-empty">
                    ...
                    <p>{{ t('No icon matches “{query}”.', { query }) }}</p>
                    <Button :label="t('Clear the search')" severity="secondary" variant="outlined" size="small" @click="clear" />
                </div>
            </div>

            <aside class="icons-panel" :aria-label="t('Selected icon')">
                <template v-if="selected">
                    <div class="icons-preview">
                        <Icon :icon="selected" :size="96" :stroke-width="stroke" />
                    </div>
                    <h2>{{ selected.name }}</h2>
                    <p class="icons-meta">{{ t(labelOf.get(selected.category ?? '') ?? '') }}</p>
                    <p class="icons-tags">
                        <span v-for="tag in selected.tags" :key="tag">{{ tag }}</span>
                    </p>
                    <div class="icons-actions">
                        <Button :label="copied === 'name' ? t('Copied') : t('Copy name')" :icon="copied === 'name' ? 'check' : 'copy'" severity="secondary" variant="outlined" size="small" @click="copy('name')" />
                        <Button :label="copied === 'snippet' ? t('Copied') : t('Copy <Icon>')" :icon="copied === 'snippet' ? 'check' : 'copy'" severity="secondary" variant="outlined" size="small" @click="copy('snippet')" />
                        <Button :label="copied === 'svg' ? t('Copied') : t('Copy SVG')" :icon="copied === 'svg' ? 'check' : 'copy'" severity="secondary" variant="outlined" size="small" @click="copy('svg')" />
                    </div>
                    <CodeBlock :code="base.has(selected.name) ? snippet : `${importLine}\n\n${snippet}`" :label="t('Usage')" lang="vue" />
                    <p v-if="!base.has(selected.name)" class="icons-note" v-html="byName" />
                </template>
                <T v-else k="iconsPage.hint" class="icons-hint">Pick an icon to copy its name, its <code>&lt;Icon&gt;</code> line or its SVG.</T>
            </aside>
        </div>

        <section class="icons-external" aria-labelledby="icons-external">
            <h2 id="icons-external">{{ t('Icons from other libraries') }}</h2>
            <T k="iconsPage.external">
            ...
            <CodeBlock :code="externalExample" :label="t('Other libraries')" lang="vue" />
        </section>
