...
export const meta: DemoMeta = {
    ...
    category: 'Data',
    description:
        'Rows and columns: `<DataGrid :value><Column field header sortable /></DataGrid>`. Sorting (Ctrl-click adds a column), a filter row, a global search, pages, selection and loading, all computed by @vitral/core, or handed to a server (`lazy`) or a data source. The reader arranges the columns themselves — width, order, pinned to an edge, shown or hidden — and what they did is one object the application can store. A real <table> with scoped headers; sort buttons, checkboxes, filters and the resize handles are native controls in the tab order.'
};
</script>
...
<template>
    ...
    >
        <div class="demo-stack" style="width: 100%; gap: 0.5rem">
            <DataGrid :value="people.slice(0, 4)" data-key="id" aria-label="This quarter" group="quarters" resizable-columns reorderable-columns size="small" scrollable class="demo-table">
            ...
            <DataGrid :value="people.slice(4, 8)" data-key="id" aria-label="Last quarter" group="quarters" resizable-columns reorderable-columns size="small" scrollable class="demo-table">
                <Column field="name" header="Name" :width="170" />
                <Column field="country" header="Country" :width="140" />
                <Column field="city" header="City" :width="150" />
                <Column field="balance" header="Balance" align="right" :width="130">
    ...
    <DemoSection title="Empty and loading">
        <DataGrid :value="[]" aria-label="Empty table" empty-message="No invoices this month" class="demo-table demo-table-half">
            <Column field="number" header="Invoice" />
            <Column field="amount" header="Amount" align="right" />
        </DataGrid>
        <DataGrid :value="people.slice(0, 4)" aria-label="Loading table" loading class="demo-table demo-table-half">
